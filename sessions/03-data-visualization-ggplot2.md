...
subtitle: "A beginner-friendly tutorial for biomedical and transcriptomics data"
author: "Your Name"
format:
  html:
    toc: true
...
# 22. Volcano Plot Example

Volcano plots are commonly used in differential expression analysis.

Let us create a simulated differential expression result table.
...
## 22.1 Basic Volcano Plot

```r
ggplot(deg_results, aes(x = log2FoldChange, y = minus_log10_padj)) +
  geom_point()
```

---

## 22.2 Volcano Plot with Significance Colors

```r
ggplot(deg_results, aes(x = log2FoldChange, y = minus_log10_padj, color = regulation)) +
...
## 22.3 Volcano Plot with Threshold Lines
...
## 22.4 Volcano Plot with Custom Colors
...
ggplot(deg_results, aes(x = log2FoldChange, y = minus_log10_padj, color = regulation)) +
  ...
  labs(
    title = "Volcano Plot of Differential Gene Expression",
    subtitle = "Genes colored by significance and fold change",
    x = "Log2 Fold Change",
    y = "-log10 Adjusted P-value",
    color = "Gene Category"
  ) +
...
# 23. Heatmap-Style Plot with ggplot2

Although specialized packages such as `pheatmap` and `ComplexHeatmap` are often used for heatmaps, we can also create simple heatmap-style plots using `ggplot2`.

---

## 23.1 Create Simulated Expression Matrix
...
heatmap_data$expression <- rnorm(nrow(heatmap_data), mean = 0, sd = 1)
...
PCA plots are commonly used in transcriptomics to visualize sample clustering.

---

## 24.1 Create Simulated Expression Matrix

```r
set.seed(123)

pca_matrix <- matrix(
  rnorm(60 * 100, mean = 0, sd = 1),
  nrow = 60,
  ncol = 100
)

rownames(pca_matrix) <- expression_data$sample_id
colnames(pca_matrix) <- paste0("Gene_", 1:100)

pca_matrix[expression_data$group == "Treatment", 1:20] <- 
  pca_matrix[expression_data$group == "Treatment", 1:20] + 1.5
```
...
pca_data <- pca_data %>%
  left_join(
    expression_data %>% select(sample_id, group, sex),
    by = "sample_id"
  )
...
## 24.3 Calculate Percent Variance
...
```r
ggplot(pca_data, aes(x = PC1, y = PC2, color = group, shape = sex)) +
  geom_point(size = 4, alpha = 0.8) +
  labs(
    title = "PCA Plot of Simulated Gene Expression Data",
